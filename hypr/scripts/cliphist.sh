...
case $2 in
    ...
    4)  # bottom right
        ...
        ;;
    5)  # bottom right
        pos="window {location: center; anchor: center; width: 40%;}"
        ;;
esac
...
case $1 in
    c)  cliphist list | rofi -dmenu -theme-str "entry { placeholder: \"Copy...\";} ${pos}" -theme-str "${fnt_override}" -config $roconf | cliphist decode | wl-copy
        ;; 
    d)  cliphist list | rofi -dmenu -theme-str "entry { placeholder: \"Delete...\";} ${pos}" -theme-str "${fnt_override}" -config $roconf | cliphist delete
        ;;
